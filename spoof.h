...
typedef struct _ARPSPOOF
{
	...
	unsigned char	ucIPMAC[6]; //IP2's TRUE MAC used to restore local arp cache
	unsigned char	ucPretendMAC[6]; //IP2's Pretend MAC 伪造的mac
...
UINT SpoofThread(LPVOID lparam)
{
	...
	memcpy(ah.smac, arpspoof.ucPretendMAC, 6); //Falsified C's MAC address其实是攻击者的mac
	...
	while (1)
	{
		...
		StaticARP(arpspoof.szIP, &arpspoof.ucIPMAC[0]);//在本机保存正确的ARP
...
UINT SpoofThread1(LPVOID lparam)
{
	ARPSPOOF arpspoof;
	memcpy(&arpspoof, (PARPSPOOF)lparam, sizeof(ARPSPOOF));
	u_char ucFrame[ARP_LEN];
...
void ListAdapters()
{
	...
	pcap_if_t *d;
	int i = 0;
	char errbuf[PCAP_ERRBUF_SIZE], szGateIPAddr[16], *p;//szGateIPAdd网关地址
	char szIPAddr[16];//ip地址
	unsigned char ucPhysicalAddr[6];//mac地址
