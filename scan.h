...
PLAN_HOST_INFO EnumLanHost(char IpAddr[], char SubMask[]);//枚举局域网内的主机

void scan_lan(LPVOID lparam)//扫描
{  	//WSADATA wsaData;
...
PLAN_HOST_INFO EnumLanHost(char IpAddr[], char SubMask[])
{
	...
	// 由子网掩码得到网段内的主机数量
	// 子网主机个数 = ~ MASK - 1
	uHostNum = ~htonl(inet_addr(SubMask)) - 1;//~取反
	...
	printf("[+] Scan for addresses from %d.%d.%d.1-%d\n\n",
		(uHostByte & 0xff000000) >> 0x18,//逻辑右移
